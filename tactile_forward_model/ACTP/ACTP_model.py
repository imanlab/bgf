...
class ACTP(nn.Module):
    ...
    def forward(self, tactiles, actions):
        ...
        outputs = []
        hidden1 = (torch.zeros(1, batch_size__, 200, device=torch.device('cuda'), dtype=torch.float32), torch.zeros(1, batch_size__, 200, device=torch.device('cuda'), dtype=torch.float32))
        hidden2 = (torch.zeros(1, batch_size__, 200, device=torch.device('cuda'), dtype=torch.float32), torch.zeros(1, batch_size__, 200, device=torch.device('cuda'), dtype=torch.float32))

        for index, (sample_tactile, sample_action,) in enumerate(zip(tactiles[:-1], actions[1:])):
            # 2. Run through lstm:
