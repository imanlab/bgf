...
class ACTP(nn.Module):
    ...
    def forward(self, tactiles, actions):
        ...
        outputs = []
        hidden1 = (torch.zeros(1, batch_size__, 200, device=torch.device('cpu'), dtype=torch.double), torch.zeros(1, batch_size__, 200, device=torch.device('cpu'), dtype=torch.double))
        hidden2 = (torch.zeros(1, batch_size__, 200, device=torch.device('cpu'), dtype=torch.double), torch.zeros(1, batch_size__, 200, device=torch.device('cpu'), dtype=torch.double))
