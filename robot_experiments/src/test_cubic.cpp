...
void trajCallback(const std_msgs::Float64MultiArray& traj)
{
  ...
  // std::cout << "Optimal v_y: " << optimal_v_y  << std::endl;
  // std::cout << "Optimal w_z: " << optimal_w_z  << std::endl;
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    // double T = 1.2; // T can change between 1.2  and 3.2
    double T = 1.3;
    double V_MAX_x = 2.1 / pow(T, 3);
    double V_MAX_z = V_MAX_x / 1.95;
   ...
   robot.setCollisionBehavior({{1000.0, 1000.0, 1000.0, 1000.0, 1000.0, 1000.0, 1000.0}},
    ...
    robot.control([=, &time, &model](const franka::RobotState& rob,
      ...
      if (time >= 1){
        v_x = -V_MAX_x * (-pow((time - T/2 - 1), 2) + pow(T, 2)/4) - 0.02;
        // v_y = (translation_y/translation_x) * v_x;
        v_y = -(translation_y/translation_x) * v_x;
        w_z = 3.5*V_MAX_x * (-pow((time - T/2 - 1), 2) + pow(T, 2)/4) + 0.01;
        // w_z = v_x - 0.01;
      }
      } else if (slip_flag_new == true){
        if (optimal_v_x != 0){
          v_x = v_x + (optimal_v_x - v_x)/500;
          v_y = v_y + (optimal_v_y - v_y)/500;
      ...
      if ((time >= 1) && (time < (T/2+1))){
        v_z = V_MAX_z * (-pow((time - T/4 - 1), 2) + pow(T, 2)/16);
      } else if((time > (T/2+1)) && (time < (T+1))){
        v_z = -V_MAX_z * (-pow((time - T/4 - T/2 - 1), 2) + pow(T, 2)/16);
      }
      }
      ...
      std::vector<double> slip_onset_data = {slip_onset_v_x, slip_onset_time};
      ...
      franka::CartesianVelocities output = {{v_x, v_y, 0, w_x, w_y, w_z}}; //Desired Cartesian velocity w.r.t. O-frame (base) {dx, dy, dz in [m/s], omegax, omegay, omegaz in [rad/s]
